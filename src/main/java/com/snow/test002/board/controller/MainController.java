...
public class MainController {
	...
	public String insertBoard(CommandMap commandMap, Model model, HttpServletRequest request) throws Exception{     
		int nAffected = mainService.insertBoard(commandMap.getMap(), request);
	...
	public @ResponseBody ReturnVO deleteBoard(CommandMap commandMap, Model model) throws Exception{     
	    ReturnVO returnVO = new ReturnVO();
		int nAffected = mainService.deleteBoard(commandMap.getMap());
		
		if ( nAffected > 0 ) {
			returnVO.setnAffected(nAffected);
		    returnVO.setMsg("삭제 성공");
		} else {
			returnVO.setnAffected(0);
			returnVO.setMsg("삭제 실패");
		}
